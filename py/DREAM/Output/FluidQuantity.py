...
class FluidQuantity(UnknownQuantity):
    ...
    def lineIntegrated(self, t=None, x0=np.array([0,10,0]), n = np.array([0,-1,0]), lmax = 20, nl = 1000, normaliseToPathLength=False):
        ...
        #if self.output.settings.radialgrid.type == rgrid.TYPE_NUMERICAL:
            #self.output.settings.radialgrid.setNumerical(self.output.settings.radialgrid.num_filename)
            #r, _, _ = self.output.settings.radialgrid.num_magneticfield.getRThetaPhiFromCartesian(x,y,z)
        ...
        if normaliseToPathLength:
            print(np.max(l[r<self.grid.a]) - np.min(l[r<self.grid.a]))
            lineIntegral /= np.max(l[r<self.grid.a]) - np.min(l[r<self.grid.a])
        return lineIntegral
