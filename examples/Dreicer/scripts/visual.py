...
def plot_growth_rates_vs_time(results):
    """
    绘制增长率随时间的变化
    """
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig('dreicer_growth_rate_vs_time.png', dpi=300)
    plt.show()

def plot_n_re_vs_time(results):
    """
    绘制逃逸电子密度随时间的变化
    """
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig('dreicer_n_re_vs_time.png', dpi=300)
    plt.show()
...
def calculate_average_growth_rates(results):
    ...
    for E_fac, do in results.items():
        ...
        if len(gamma) > 10:  # 确保有足够的数据点
            ...
            print(f"E/Ec = {E_fac:4.1f}: Average growth rate = {avg_gamma:.4e} s⁻¹")
    
    return np.array(E_factors), np.array(avg_growth_rates)

def plot_growth_rate_comparison(E_factors, avg_growth_rates):
    """
    绘制平均增长率与E/Ec的关系
    """
    fig, ax = plt.subplots(1, 1, figsize=(10, 6))
    ...
    plt.tight_layout()
    plt.savefig('dreicer_avg_growth_rates.png', dpi=300)
    plt.show()
