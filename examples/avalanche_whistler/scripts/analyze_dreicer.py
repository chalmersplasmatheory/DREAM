...
def analyze_dreicer_generation(data_file, plot_dir='../figures'):
    """
    分析Dreicer生成率
    """
    # 加载模拟结果
    do = DREAMOutput(data_file)
    ...
    plt.savefig(os.path.join(plot_dir, 'dreicer_analysis.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    主函数
    """
    parser = argparse.ArgumentParser(description='Analyze Dreicer generation results')
    ...
    print(f"Analyzing Dreicer generation results from {args.data_dir}...")
    
    try:
        avg_rate, E_ratio = analyze_dreicer_generation(args.data_dir, args.plot_dir)
        print(f"Analysis completed. Results saved to '{args.plot_dir}/dreicer_analysis.png'")
