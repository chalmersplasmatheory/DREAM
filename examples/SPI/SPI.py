...
Nt_init = 20         # number of time steps
...
ds.eqsys.n_i.addIon(name='D_inj', Z=1, isotope=2, iontype=Ions.IONS_DYNAMIC_NEUTRAL, n=density_D_inj, SPIMolarFraction=1)


# Set E_field 
efield = E_initial*np.ones((len(times), len(radius)))
ds.eqsys.E_field.setPrescribedData(efield=efield, times=times, radius=radius)
ds.eqsys.E_field.setBoundaryCondition(R0=R0)
...
# Set temperature
temp_prof=((1-0*0.75*(radialgrid/radialgrid[-1])**2)**2).reshape(1,-1)
...
ds.eqsys.spi.setShift(True, T=np.array([5, 5]), T0=2, delta_y = 0.0125, Rm=R0)
...
ds.save('SPI_init_settings.h5')
runiface(ds, 'SPI_output.h5', quiet=False)
...
# Save settings to HDF5 file
ds.save('SPI_init_settings2.h5')
runiface(ds, 'SPI_output2.h5', quiet=False)
...
ds2.fromOutput('SPI_output2.h5',ignore=['v_p'])
ds2.eqsys.spi.setInitialData(vp=vp_init)

ds2.eqsys.E_field.setType(Efield.TYPE_SELFCONSISTENT)
ds2.eqsys.E_field.setBoundaryCondition(bctype = Efield.BC_TYPE_PRESCRIBED, inverse_wall_time = 0, V_loop_wall_R0 = E_wall*2*np.pi, R0=R0)
...
ds2.save('injection_restart_settings.h5')
runiface(ds2, 'output_restart_injection.h5', quiet=False)
