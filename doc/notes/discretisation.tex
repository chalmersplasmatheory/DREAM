...
\begin{document}
    ...
    the Newton solver may overshoot and first oscillate around the solution,
    or even diverge completely. One method of accelerating convergence is given
    in {\em Numerical Recipes} and is based on the optimization of an equivalent
    one-dimensional problem. The purpose of the method is to determine the
	optimal damping factor $\lambda$ such that the Newton update
    \begin{equation}
        \bb{x}^{(k+1)} = \bb{x}^{(k)} + \lambda\delta\bb{x},
    \end{equation}
    with $\delta\bb{x}\equiv\Jac^{-1}\bb{F}$, takes us as close to the true
    ...
    may however be too long and actually lead to an increase in $f$ if taken
    ...
    \begin{equation}
        g(\lambda) = f\left(\bb{x}^{(k)} + \lambda\delta\bb{x}\right)
			= f\left(\bb{x}^{(k+1)}\right).
    \end{equation}
    This in turn implies that the derivative of this function with respect to
	$\lambda$ is
    \begin{equation}
        g'(\lambda) = \nabla f\cdot\delta\bb{x}.
    \end{equation}
