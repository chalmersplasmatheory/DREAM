...
&= \frac{n\sub{tot}}{\mathcal{V}'_{ij}}\frac{1}{\Delta p}\int_{p_{i-1/2}}^{p_{i+1/2}}\rd p \int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\frac{p}{\gamma}\Sigma(\gamma,\,\gamma_1)\Delta_j(p,\,p_1) \nonumber \\ 
\Delta_j &= \frac{1}{\Delta \xi_j}\int_{\xi_{j-1/2}}^{\xi_{j+1/2}}\rd \xi_0 \mathcal{V}'(r,\,p,\,\xi_0)\{\Delta\}.
\end{align}
...
S &=  2\pi r_0^2 c\frac{\gamma_1^2}{\gamma_1^2-1}\left(\frac{1}{\gamma_1-\gamma} - \frac{1}{\gamma-1}\right) 
+ \frac{\gamma}{\gamma_1^2-1} 
- \frac{2\gamma_1 - 1}{(\gamma_1-1)(\gamma_1^2-1)}\ln\frac{\gamma-1}{\gamma_1-\gamma},
\end{align}
we can write
\begin{align}
C_{ij} &\approx \frac{n\sub{tot}}{\mathcal{V}'_{ij}}\frac{1}{\Delta p_i}\int_{p_{i-1/2}}^{p_{i+1/2}}\rd p \int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\frac{\partial S}{\partial p}\Delta_{ij}
\nonumber \\
&= \frac{n\sub{tot}}{\mathcal{V}'_{ij}}\int_{p_0(\rho_1)}^\infty \rd p_1 \, \frac{p_1}{\gamma_1}\frac{S(\rho_2) - S(\rho_1)}{\Delta p_i}\Delta_{ij}(p_1), \nonumber \\
...
\begin{align}
\Delta_j =  \frac{1}{\Delta \xi_j} \frac{1}{\pi} \int_{\xi_{j-1/2}}^{\xi_{j+1/2}}\rd \xi_0 \oint \rd\theta \int_{\cos(\theta_p+\theta^\star)}^{\cos(\theta_p-\theta^\star)}\rd \xi_1 \frac{\sqrt{g}(r,\,\theta,\,p_1,\,\xi)f(r,\,\theta,\,p_1,\,\xi_1)}{\sqrt{1-{\xi^\star}^2-\xi_1^2-\xi^2+2\xi^\star\xi_1\xi}},
\end{align}
...
to reformulate
\begin{align}
\Delta_j(p,\,p_1) &= \int_{\zeta_{j1}}^{\zeta_{j2}} \rd \xi_{0,1} \, \mathcal{V}'(r,\,p_1,\,\xi_{0,1})f(r,\,p_1,\,\xi_{0,1})\delta_j \\
\delta_j(p,\,p_1,\,\xi_{0,1}) &= \frac{1}{\mathcal{V}'(r,\,p_1,\,\xi_{0,1})\Delta \xi_j} \oint \rd \theta \sqrt{g}(r,\,\theta,\,p_1,\,\xi_1) D_j, \\
...
\end{align}
Notably, when $\xi_1=1$, we get $D \equiv 1$. The integration limits here are quite tedious to derive, but are given by
\begin{align}
...
0,& \mathrm{otherwise}
\end{cases} \nonumber \\
t_2 &= \xi(x_2,\,\theta) = \xi(\xi_{0, j+1/2},\,\theta),
\end{align}
and 
\begin{align}
...
The Chiu-Harvey approximation is obtained by assuming
\begin{align}
f(r,\,p,\,\xi_0) &= \frac{F(p)}{\mathcal{V}'(r,\,p,\,\xi_0)} \delta(\xi_0-1) \\
F(p) &= \int_{-1}^1 \rd \xi_0 \,\mathcal{V}'f,
\end{align}
in which case
\begin{align}
...
\end{cases}
\end{align}
where $\zeta_{j2}=1$ if $t_1 \leq \xi^\star \leq t_2$ for any $\theta$, and 
\begin{align}
\delta_j(p,\,p_1,\,1) &= \frac{1}{\mathcal{V}'\Delta \xi_j}\int_{\theta_1}^{\theta_2}\rd \theta \,\sqrt{g},
\end{align}
integrated over all $\theta_{b1}(\xi_{j+1/2}) < \theta < \theta_{b2}(\xi_{j+1/2})$ for which $\xi_{j-1/2} < \xi_0(\xi^\star,\,\theta) < \xi_{j+1/2}$.

The Rosenbluth-Putvinski approximation is further obtained by approximating 
\begin{align}
F(p) =  \delta(p-\infty) V'\frac{n\sub{RE}}{B}\langle B \rangle,
\end{align}
