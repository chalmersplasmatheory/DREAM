...
t_1 &=  \begin{cases}
\xi(\xi_{0, j-1/2},\,\theta), & |\xi_{0, j-1/2}| > \sqrt{1-\frac{B\sub{min}}{B(\theta)}} \\
0,& \mathrm{otherwise}
\end{cases} \nonumber \\
t_2 &= \xi(\xi_{0, j+1/2},\,\theta).
\end{align}
...
\end{cases} 
\end{align}
Furthermore, the $\oint \rd \theta$ integral is taken over those $\theta \in [\theta_{b1}(\xi_{j+1/2}), \, \theta_{b2}(\xi_{j+1/2}]$  for which $u_1(\theta) < \xi_{01} < u_2(\theta)$, e.g.
\begin{align}
\oint \rd \theta \,A = \int_{\theta_{b1}(\xi_{j+1/2})}^{\theta_{b2}(\xi_{j+1/2})}\rd \theta  \times\begin{cases}
A, & u_1(\theta) < \xi_{01} < u_2(\theta) \\
0, & \mathrm{otherwise}.
\end{cases}
\end{align}
Note, in these expressions, we have not described how to add the ``$-\xi$'' branch of the negative-trapped region to the overall source term. These are obtained by flipping the sign of the non-square-root terms in $z_1$, $z_2$, $y_1$ and $y_2$. 
