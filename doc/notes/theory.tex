...
The electron-impact ionization cross section of a bound electron in subshell $i$ in an ion with charge number $Z_0$ is given by
\begin{align}
\sigma_{Z_0,i \to  Z_0+1}& = (1-S(p))\sigma_{Z_0,i \to  Z_0+1}^\text{NR} + S(p) \sigma_{Z_0,i\to Z_0+1}^\text{R}, \nonumber \\
S(p) &= \frac{1}{1+ \exp[ 1 - (\sqrt{m_e^2 c^4+c^2 p^2}-m_e c^2)/(10^5\,\text{eV})]}, \nonumber \\
\sigma_{Z_0,i\to Z_0+1}^\text{NR} &= \pi a_0^2 C \mathcal{N}_i \left(\frac{\text{Ry}}{\Delta I_Z^i}\right)^2\frac{(\ln U)^{1+\beta^\star/U}}{U} \nonumber \\
U &= \frac{p^2}{2m\Delta I_Z^i}, \nonumber \\
...
\sigma_{Z_0,i\to Z_0+1}^\text{R} &= \pi r_0^2 C \mathcal{N}_i \left(\frac{\text{Ry}}{\Delta I_Z^i}\right) \left[\ln\left(\frac{p^2}{2m\Delta I_Z^i}\right) - \frac{p^2}{\sqrt{m^2c^2+p^2}}\right],
\end{align}
where $\text{Ry} \approx 13.6\,$eV is the Rydberg energy, $C \approx 2$ an undetermined constant, $\mathcal{N}_i$ the number of electrons in subshell $i$ (for neutral argon, $\mathcal{N}_1 = 2$ for the two electrons in the 1s state, $\mathcal{N}_2 = 2$ for the two electrons in 2s, $\mathcal{N}_3=6$ for the 2p shell, $\mathcal{N}_4 = 2$ and $\mathcal{N}_5=6$), and $\Delta I_Z^i$ is the ionization threshold energy for the subshell $i$; this data for the atomic binding energy of electrons in each shell of all ionization states of impurities from $Z=1$ to $Z=30$ is given in [W Lotz, \emph{Subshell Binding Energies of Aoms and Ions from Hydrogen to Zinc}, Journal of the Optical Society of America {\bf 58} (1968)]. This model uses the simpler hydrogenic shell model for the effective charge number $Q_n$ of the shell with principal quantum number $n$ and corresponding occupation number $P_n$ (where, for neutral argon, $P_1=2$, $P_2 = 8$ and $P_3 = 8$), such that the total energy of the ion is approximatetly $E\sub{ion}(\text{Ry}) \approx - \sum_i Q_n^2\mathcal{N}_n/n^2$. In the expression for $Q_n$, $\sigma(i,j)$ are hydrogenic screening constants which are given in [R Marchand, S Caille. ``Improved screening coefficients for the hydrogenic ion model''. Journal of Quantitative Spectroscopy and Radiative Transfer {\bf 43} (1990)], and presented in the table below.
...
The heat transport of the cold fluid electrons, $W = (3/2)n\sub{cold}T_{e,\text{cold}}$, is governed by the equation
\begin{align}
\frac{\partial W}{\partial t} &= E_\parallel j_\Omega + \int  \frac{p^2}{2m_e}\nu_{E,ee} f \,\rd \b{p}  + n\sub{RE} ceE_c^\text{(cold)} -n_e \sum_Z \sum_i n_{Zi}L_{Zi}(T,\,n\sub{cold})  \nonumber \\
&+ \frac{1}{V'}\frac{\partial}{\partial r}\left[V'\left(-A_W W + D_W\frac{\partial W}{\partial r}\right)\right], \nonumber \\
...
\end{align}
plus terms that vanish upon flux surface averaging. $E_c^\text{(cold)}$ here should be evaluated with the Coulomb logarithm characteristic for the runaway populaltion. The flux surface average of this yields, inserting $j_\Omega/B = \sigma \langle \b{E}\cdot\b{B}\rangle/\langle B^2\rangle$, 
\begin{align}
\frac{\partial W}{\partial t} &= \sigma \frac{\langle \b{E}\cdot\b{B}\rangle^2}{\langle B^2\rangle}+ \left\langle\int  \frac{p^2}{2m_e}\nu_{E,ee} f \,\rd \b{p}\right\rangle + \frac{j\sub{RE}}{B}\langle B\rangle E_c^\text{(cold)} -n_e \sum_Z \sum_i n_{Zi}L_{Zi}(T,\,n\sub{cold})   \nonumber \\
&+ \frac{1}{V'}\frac{\partial}{\partial r}\left[V'\left(-A_W W + D_W\frac{\partial W}{\partial r}\right)\right].
\end{align}
...
\textcolor{red}{[add heat exchange with hydrogen species, which initially carries a large amount of kinetic energy -- perhaps ignore pathway via impurities?]}
...
&< \frac{\pi e }{B\sub{max}} \int_0^\infty \rd p \,vp^2f.
\end{align}
