...
Avalanche generation via knock-on collisions are described by the linearized field-particle boltzman collision integral, describing the influence of binary large-angle collisions due to fast electrons on a stationary field particle distribution\footnote{Embréus, O., Stahl, A. and Fülöp, T., 2018. \emph{On the relativistic large-angle electron collision operator for runaway avalanches in plasmas}. Journal of Plasma Physics, \b{84}(1), p.905840102.}
\begin{align}
C\sub{ava} &= \frac{n\sub{tot}}{p\gamma}\int_{p_0}^\infty\rd p_1\,\frac{p_1^3}{\gamma_1}\Sigma(\gamma,\,\gamma_1)\frac{1}{2\pi}\int\rd\xi_1\rd\varphi_1\,\delta(\xi_s-\xi^\star)f(\mathbf{p}_1), \nonumber \\
&\equiv n\sub{tot}\frac{p}{\gamma}\int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\Sigma(\gamma,\,\gamma_1)\Delta(p,\,p_1,\,\xi),
\\
\{C\sub{ava}\} &= n\sub{tot}\frac{p}{\gamma}\int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\Sigma(\gamma,\,\gamma_1)\{\Delta\}, \\
\xi_s &= \frac{\mathbf{p}_1\cdot\mathbf{p}}{p_1 p}, \\
\xi^\star &= \sqrt{\frac{\gamma_1+1}{\gamma_1-1}\frac{\gamma-1}{\gamma+1}} = \frac{\gamma_1+1}{\gamma+1}\frac{p}{p_1}, \\
\gamma_0 &= 	2\gamma-1, \nonumber \\
p_0 &= \sqrt{\gamma_0^2 - 1} = 2\sqrt{\gamma}\sqrt{\gamma-2}, \\
\Sigma(\gamma,\,\gamma_1) &= 2\pi r_0^2 c \frac{\gamma_1^2}{(\gamma_1^2-1)(\gamma-1)^2(\gamma_1-\gamma)^2}\biggl(
(\gamma_1-1)^2 - \frac{(\gamma-1)(\gamma_1-\gamma)}{\gamma_1^2} \nonumber\\
&\quad\times \left[2\gamma_1^2 + 2\gamma_1 - 1 - (\gamma-1)(\gamma_1-\gamma)\right]
\biggr).
\end{align}
Here, $n\sub{tot}$ is the total background electron density (bound + ionized, assumed to be constant on flux surfaces), since typical momentum transfers for runaway generation ($pc \sim 0.5\,\mathrm{MeV}/\sqrt{E/E_c - 1}$) most often far exceed characteristic ionization energies. For the most tightly bound electron in argon, this is true up to $E/E_c \sim 10^4$. For the same reason, we have neglected the thermal speeds of the target electrons, taking them as stationary ($p=0$).

Furthermore, we also assume there to exist a cut-off momentum $p_c$, such that for $p<p_c$, $C\sub{ava}\equiv 0$. This is done so as not to double count small-angle collisions -- the collision term above will create particles (and momentum) at a diverging rate as $p\to 0$; ideally a complementary collision term accounting for the slowing-down of the incident electron would be added to counteract this effect, where the small-angle tail of the combined operator is the Fokker-Planck operator we know and love. However, to capture avalanche generation, a reasonably large $p_c$ can be chosen in such a way that the total momentum and energy generated by $C\sub{ava}$ is negligible. Indeed, only a small number of electrons are scattered with such large energy transfers, but this number can still provide a dominant runaway-generation channel despite having negligible impact on the bulk electron dynamics.

Now, the challenge of discretizing the avalanche knock-on operator lies in describing the double gyro- and bounce averaged delta function
\begin{align}
\{\Delta\} &= \frac{1}{2\pi}\frac{p_1^2}{p^2}\frac{1}{\mathcal{V}'(r,\,p,\,\xi_0)}\int_0^{2\pi} \rd \phi \int_0^{2\pi} \rd \zeta \oint \rd\theta \,\sqrt{g}(r,\,\theta,\,p,\,\xi)\int \rd\xi_1\rd\varphi_1 \,\delta(\xi_s-\xi^\star)f(\mathbf{p}_1) \nonumber \\
&= \frac{1}{\pi}\frac{(2\pi)^2}{\mathcal{V}'(r,\,p,\,\xi_0)}\oint \rd\theta \,\sqrt{g}(r,\,\theta,\,p_1,\,\xi)\int_{\cos(\theta_p+\theta^\star)}^{\cos(\theta_p-\theta^\star)}\,\frac{\rd \xi_1 \, f(r,\,\theta,\,p_1,\,\xi_1)}{\sqrt{1-{\xi^\star}^2-\xi_1^2-\xi^2+2\xi^\star\xi_1\xi}},
\end{align}
where $\cos\theta^\star \equiv \xi^\star$ and we utilized the simple momentum dependence of the metric $\sqrt{g}(p)\propto p^2$. 

\subsubsection{Finite-volume discretization}
The finite-volume discretization of the knock-on term is
\begin{align}
C_{ij} &= \frac{\int_{p_{i-1/2}}^{p_{i+1/2}}\rd p \int_{\xi_{j-1/2}}^{\xi_{j+1/2}}\rd\xi_0\,\mathcal{V}'(r,\,p,\,\xi_0)\{C\sub{ava}\}}{\Delta p_i \Delta \xi_j \mathcal{V}'_{ij}} \nonumber \\
&= n\sub{tot}\frac{V'}{\mathcal{V}'_{ij}}\frac{1}{\Delta p}\int_{p_{i-1/2}}^{p_{i+1/2}}\rd p \int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\frac{p}{\gamma}\Sigma(\gamma,\,\gamma_1)\Delta_j(p,\,p_1) \nonumber \\ 
\Delta_j &= \frac{1}{V' \Delta \xi_j}\int_{\xi_{j-1/2}}^{\xi_{j+1/2}}\rd \xi_0 \mathcal{V}'(r,\,p,\,\xi_0)\{\Delta\}.
\end{align}
This is the exact cell-averaged source term. If we now make the assumption that
\begin{align}
\Delta_j(p,\,p_1) \approx \Delta_{ij} \equiv \Delta_j(p_i,\,p_1),
\end{align}
e.g. that the knock-on source takes the pitch dependence of the mid-point in the momentum cell, and utilize the flux form of the scattering rate
\begin{align}
\frac{p}{\gamma}\Sigma &= \frac{\partial S}{\partial p} \\
S &=  \frac{2\pi r_0^2 c}{\gamma_1^2-1}\biggl[ \gamma_1^2\left(\frac{1}{\gamma_1-\gamma} - \frac{1}{\gamma-1}\right) 
+ \gamma \nonumber \\
&\hspace{18mm} - \frac{2\gamma_1 - 1}{\gamma_1-1}\ln\frac{\gamma-1}{\gamma_1-\gamma} \biggr],
\end{align}
we can exchange integration order of the momentum integrals and obtain
\begin{align}
C_{ij} &\approx n\sub{tot}\frac{V'}{\mathcal{V}'_{ij}}\frac{1}{\Delta p_i}\int_{p_{i-1/2}}^{p_{i+1/2}}\rd p \int_{p_0}^\infty\rd p_1\,\frac{p_1}{\gamma_1}\frac{\partial S}{\partial p}\Delta_{ij}
\nonumber \\
&= n\sub{tot}\frac{V'}{\mathcal{V}'_{ij}}\int_{p_0(\rho_1)}^\infty \rd p_1 \, \frac{p_1}{\gamma_1}\frac{S(\rho_2) - S(\rho_1)}{\Delta p_i}\Delta_{ij}(p_1), \nonumber \\
\rho_1 &= \max\left(p_{i-1/2}, ~p\sub{cut}\right) \\
\rho_2 &= \min\left[p_{i+1/2}, ~ \sqrt{ \left(\frac{1+\gamma_1}{2} \right)^2 - 1} \right].
\end{align}
It remains to massage the angle-dependence term:
\begin{align}
\Delta_j =  \frac{(2\pi)^2}{V' \Delta \xi_j} \frac{1}{\pi} \int_{\xi_{j-1/2}}^{\xi_{j+1/2}}\rd \xi_0 \oint \rd\theta \int_{\cos(\theta_p+\theta^\star)}^{\cos(\theta_p-\theta^\star)}\rd \xi_1 \frac{\sqrt{g}(r,\,\theta,\,p_1,\,\xi)f(r,\,\theta,\,p_1,\,\xi_1)}{\sqrt{1-{\xi^\star}^2-\xi_1^2-\xi^2+2\xi^\star\xi_1\xi}},
\end{align}
where we can repeatedly exchange integration orders and use 
\begin{align}
\rd \xi_1 \rd \xi_0 \sqrt{g}(p_1,\,\xi)) = \rd \xi_1 \rd \xi \, p_1^2 \mathcal{J} = \rd \xi \rd \xi_{0,1}\sqrt{g}(p_1,\,\xi_1)
\end{align}
to reformulate
\begin{align}
\Delta_j(p,\,p_1) &= \frac{1}{V'}\int_{\zeta_{j1}}^{\zeta_{j2}} \rd \xi_{0,1} \, \mathcal{V}'(r,\,p_1,\,\xi_{0,1})f(r,\,p_1,\,\xi_{0,1})\delta_j \\
\delta_j(p,\,p_1,\,\xi_{0,1}) &= \frac{(2\pi)^2}{\mathcal{V}'(r,\,p_1,\,\xi_{0,1})\Delta \xi_j} \oint \rd \theta \sqrt{g}(r,\,\theta,\,p_1,\,\xi_1) D_j, \\
D_j(p,\,p_1,\,\xi_{0,1},\,\theta) &= \frac{1}{\pi}\int_{y_{j1}}^{y_{j2}}\frac{\rd\xi}{\sqrt{1-{\xi^\star}^2-\xi_1^2-\xi^2+2\xi^\star\xi_1\xi}} \nonumber \\
&= F(y_{j2}) - F(y_{j1}), \\
F(y) &= \frac{1}{\pi}\arcsin \frac{y-\xi_1\xi^\star}{\sqrt{(1-\xi_1^2)(1-\xi^{\star 2})}}.
\end{align}

The integration limits here are quite tedious to derive, but are given by
\begin{align}
y_{j1} &= \max\left[t_1,~\xi^\star \xi_1 - \sqrt{1-\xi^{\star 2}}\sqrt{1-\xi_1^2}\right] ,\nonumber \\
y_{j2} &= \min\left[t_2,~\xi^\star \xi_1 + \sqrt{1-\xi^{\star 2}}\sqrt{1-\xi_1^2}\right], \nonumber \\
t_1 &= \begin{cases}
0, & 0 \leq \xi_{j-1/2} \leq \sqrt{1-\frac{B\sub{min}}{B(\theta)}}, \\
\xi(\xi_{0, j-1/2},\,\theta),& \mathrm{otherwise}
\end{cases} \nonumber \\
t_2 &= \begin{cases}
0, & -\sqrt{1-\frac{B\sub{min}}{B(\theta)}} \leq \xi_{j+1/2} \leq 0, \\
\xi(\xi_{0, j+1/2},\,\theta),& \mathrm{otherwise}.
\end{cases} \nonumber
\end{align}
Inserting these definitions, we can simplify
\begin{align}
F(y_{j2})  &= \begin{cases}
\frac{1}{2}, & t_2 \geq \xi^\star \xi_1 + \sqrt{1-\xi^{\star 2}}\sqrt{1-\xi_1^2}, \\
F(t_2), & \mathrm{otherwise},
\end{cases} \nonumber\\
F(y_{j1}) &= \begin{cases}
-\frac{1}{2}, & t_1 \leq \xi^\star \xi_1 - \sqrt{1-\xi^{\star 2}}\sqrt{1-\xi_1^2}, \\
F(t_1), & \mathrm{otherwise},
\end{cases} \nonumber
\end{align}
and $D\equiv 1$ when $\xi_1=1$. We also have
\begin{align}
\zeta_{j1} &= \min_{\theta\in[\theta_1,\,\theta_2]} u_1, \nonumber \\
\zeta_{j2} &= \max_{\theta\in[\theta_1,\,\theta_2]}u_2, \nonumber  \\
u_1 &= \xi_0(z_1,\,\theta), \nonumber \\
u_2 &= \xi_0(z_2,\,\theta), \nonumber \\
z_1 &= \begin{cases}
t_2 \xi^\star - \sqrt{1-t_2^2} \sqrt{1-\xi^{\star 2}}, & t_2 < -\xi^\star \\
-1, & t_1 \leq -\xi^\star \leq t_2\\
t_1 \xi^\star - \sqrt{1-t_1^2} \sqrt{1-\xi^{\star 2}}, & -\xi^\star < t_1 \\
\end{cases} \nonumber \\
z_2 &= \begin{cases}
t_2 \xi^\star + \sqrt{1-t_2^2} \sqrt{1-\xi^{\star 2}}, & t_2 < \xi^\star \\
1, & t_1 \leq \xi^\star \leq t_2\\
...
\end{cases} 
\end{align}
Furthermore, the $\oint \rd \theta$ integral is taken over those $\theta \in [\theta_1, \, \theta_2]$  for which $u_1(\theta) < \xi_{01} < u_2(\theta)$ and $|\xi_{01}| \geq \sqrt{1 - B\sub{min}/B}$, e.g.
\begin{align}
\oint \rd \theta \,A &= \int_{\theta_1}^{\theta_2}\rd \theta  \times\begin{cases}
A, & u_1(\theta) < \xi_{01} < u_2(\theta), \\
0, & \mathrm{otherwise}, 
\end{cases}\nonumber \\
\theta_1 &= \max\left[\theta_{b1}(\xi_{01}),\,\theta_{b1}(\xi_{j-1/2}),\,\theta_{b1}(\xi_{j+1/2})\right],
\nonumber \\
\theta_2 &= \min\left[\theta_{b2}(\xi_{01}),\, \theta_{b2}(\xi_{j-1/2}),\,\theta_{b2}(\xi_{j+1/2})\right], \nonumber
\end{align}

\paragraph{Trapping corrections:}
In these expressions, we have not yet accounted for the fact that the particle phase space in \textsc{dream} does not extend to the negative trapped region; instead such contributions should be added to the mirrored (positive) pitch. Therefore, the true $\delta_j$ to use in \textsc{dream} is
\begin{align}
\begin{cases}
\delta_j(\xi^\star,\,\xi_{01}), & \text{$\xi_0$ and $\xi_{01}$ both passing} \\
\delta_j(\xi^\star,\,\xi_{01}) + \delta_j(\xi^\star,\,-\xi_{01})& \text{$\xi_0$ passing, $\xi_{01}$ trapped}\\
\delta_j(\xi^\star,\,\xi_{01}) + \delta_j(-\xi^\star,\,\xi_{01})& \text{$\xi_0$ trapped, $\xi_{01}$ passing}\\
\delta_j(\xi^\star,\,\xi_{01}) + \delta_j(\xi^\star,\,-\xi_{01})+ \delta_j(-\xi^\star,\,\xi_{01})+ \delta_j(-\xi^\star,\,-\xi_{01})& \text{$\xi_0$ and $\xi_{01}$ both trapped}
\end{cases}
\end{align}
Now, thanks to the symmetry property $\delta(-x,\,-y) = \delta(x,\,y)$ it follows that this is equivalent to
\begin{align}
\begin{cases}
\delta_j(\xi^\star,\,\xi_{01}), & \text{$\xi_0$ and $\xi_{01}$ both passing} \\
\delta_j(\xi^\star,\,\xi_{01}) + \delta_j(\xi^\star,\,-\xi_{01})& \text{one of $\xi_0$ or $\xi_{01}$ trapped}\\
2\left[\delta_j(\xi^\star,\,\xi_{01}) + \delta_j(\xi^\star,\,-\xi_{01})\right]&\text{$\xi_0$ and $\xi_{01}$ both trapped}\\
\end{cases}
\end{align}


\subsubsection{Chiu-Harvey and Rosenbluth-Putvinski approximations}
The approximate knock-on terms are obtained by using idealized distribution functions. By noting that the runaway-electron density is 
\begin{align}
\langle n\sub{RE}\rangle = \frac{1}{V'} \int \rd p \int \rd \xi_0 \, \mathcal{V}' f,
\end{align}
we can first retrieve the Chiu-Harvey approximation as the unit-pitch limit
\begin{align}
f(r,\,p,\,\xi_0) &= \frac{V'(r)}{\mathcal{V}'(r,\,p,\,\xi_0)}F(p) \delta(\xi_0-1) \\
F(p) &= \frac{1}{V'}\int_{-1}^1 \rd \xi_0 \,\mathcal{V}'f,
\end{align}
such that $\langle n\sub{RE}\rangle = \int F(p)\,\rd p$,
in which case
\begin{align}
\Delta_j =  F(p) \delta_j(p,\,p_1,\,1) \times \begin{cases}
1, & \zeta_{j2}=1 \\
0, & \mathrm{otherwise}
\end{cases}
\end{align}
where $\zeta_{j2}=1$ iff $t_1 \leq \xi^\star \leq t_2$ for any $\theta$, and 
\begin{align}
\delta_j(p,\,p_1,\,1) &= \frac{(2\pi)^2}{\mathcal{V}'(r,\,p_1,\,\xi_{0,1}=1)\Delta \xi_j}\int_{\theta_1}^{\theta_2}\rd \theta \,\sqrt{g}(r,\,\theta,\,p_1,\,\xi_1=1) \nonumber \\
&= \frac{1}{\Delta \xi_j} \frac{\int_{\theta_1}^{\theta_2}\rd \theta \,\mathcal{J}B}{\int_{-\pi}^{\pi}\rd \theta \,\mathcal{J}B}
\end{align}
integrated over all $\theta_{b1}(\xi_{j+1/2}) < \theta < \theta_{b2}(\xi_{j+1/2})$ for which $\xi_{j-1/2} < \xi_0(\xi^\star,\,\theta) < \xi_{j+1/2}$.
Consequently, the Chiu-Harvey collision term can be formulated as
\begin{align}
C_{ij}^{(\mathrm{CH})} = n\sub{tot}\frac{V'}{\mathcal{V}'_{ij}}\int_{p_0(\rho_1)}^\infty \rd p_1 \, \frac{p_1}{\gamma_1}\frac{S(\rho_2) - S(\rho_1)}{\Delta p_i} F(p_1) \frac{1}{\Delta \xi_j} \frac{\int_{\theta_1}^{\theta_2}\rd \theta \,\mathcal{J}B}{\int_{-\pi}^{\pi}\rd \theta \,\mathcal{J}B}.
\end{align}

The Rosenbluth-Putvinski approximation is obtained by further simplifying
\begin{align}
F(p) = \langle n\sub{RE} \rangle \delta(p-\infty) = \frac{n\sub{RE}}{B}\langle B \rangle \delta(p-\infty),
\end{align}
in which case we get to use
\begin{align}
S(p,\,p_1=\infty) &= -\frac{2\pi r_0^2 c}{\gamma-1}.
\end{align}
This gives a relatively simple form for the Rosenbluth-Putvinski term
\begin{align}
C_{ij}^\mathrm{(RP)} = n\sub{tot}\frac{n\sub{RE}}{B}\langle B \rangle \frac{V'}{\mathcal{V}'_{ij} }\frac{2\pi r_0^2 c}{\Delta p_i\Delta \xi_j}\left(\frac{1}{\gamma(\rho_1)-1} - \frac{1}{\gamma(\rho_2)-1}\right) \frac{\int_{\theta_1}^{\theta_2}\rd \theta \,\mathcal{J}B}{\int_{-\pi}^{\pi}\rd \theta \,\mathcal{J}B}.
\end{align}

In these expressions the trapping corrections from the general case do not need to be carried out; $\xi_{01}=1$ is always passing, and since $\xi^\star>0$ the knock-ons always have positive pitch.

\subsubsection{Total production rate}
An analytic integration shows that 
\begin{align}
\int_{-1}^1 \rd \xi \, \Delta(r,\,\theta,\,p,\,p_1,\,\xi) = \int_{-1}^1\rd \xi_1 \, f(r,\,\theta,\,p_1,\,\xi_1)
\end{align}
or, equivalently for the bounce average,
\begin{align}
\int_{-1}^1 \rd \xi_0 \, \mathcal{V}'(r,\,p,\,\xi_0)\{\Delta\}(r,\,p,\,p_1,\,\xi_0) = \int_{-1}^1\rd\xi_{01}\,\mathcal{V}'(r,\,p_1,\,\xi_{01}) f(r,\,p_1,\,\xi_{01}).
\end{align} 
In terms of the discretized version,
\begin{align}
\Delta_j = \frac{1}{V'}\sum_l \Delta \xi_l \mathcal{V}'(r,\,p_1,\,\xi_l) f(r,\,p_1,\xi_l) \delta_{jl}
\end{align}
where $\delta_{jl} = \delta_j(p,\,p_1,\,\xi_l)$ (possibly containing corrections at the end points of the interval), the above condition takes the form
\begin{align}
\sum_j \Delta \xi_j V' \Delta_j \equiv  \sum_l \Delta \xi_l \mathcal{V}'(r,\,p_1,\,\xi_l) f(r,\,p_1,\xi_l) \sum_j \Delta \xi_j \delta_{jl} = \sum_l \Delta\xi_l \mathcal{V}'_l f_l.
\end{align}
Therefore, we should expect
\begin{align}
\sum_{j=1}^{N_\xi} \Delta \xi_j\delta_{jl} = 1
\end{align}
to hold for each $l$. This can be used not only as validation of the implementation of $\delta_j$, but also to correct for quadrature errors in the evaluation of $\delta$ to ensure that the correct total number of knock-ons are always produced, regardless of resolution (although the distribution within each cell may be skewed). That is, in the definition of the discretized $\Delta_j$ we could replace $\delta_{jl}$ by $\delta_{jl}/\sum_{j=1}^{N_\xi} \Delta \xi_j\delta_{jl}$ to ensure particle ``conservation''.
