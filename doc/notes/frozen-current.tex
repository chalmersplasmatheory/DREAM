...
\begin{document}
	...
	\red{We could use $\gamma_{\rm primary}+\Gamma_{\rm ava}n_{\rm re}$ as floor
	value otherwise\ldots By taking $D_0$ such that the transport exactly
	balances the generation, and then applying
	$D=D_0+\delta D(I_{\rm p}-I_{\rm p,presc})$, for some function $\delta D$ of
	the $\Delta I_{\rm p}$}.
	...
	%
	% - D0 + dD when Ip is close (but not equal to) prescribed value
	% - Under which circumstances does equation (17) become infinite?
	%
	...
	\begin{equation}
		\begin{gathered}
			\int_0^a \frac{G}{B_{\rm min}}\left\langle \frac{1}{R^2} \right\rangle
				...
				%= \frac{1}{R_0^3}\left[\arcsin{\epsilon} - \sqrt{1-\epsilon^2}\right]_{\epsilon=0}^1
				%
	...
	\red{(Needs to be updated with new version of~\eqref{eq:dD_tanh}, which
	takes into account the $\partial I_\Omega/\partial t$ term)}
	\begin{equation}
		\begin{aligned}
			\frac{\partial\delta D}{\partial x_i} &=
