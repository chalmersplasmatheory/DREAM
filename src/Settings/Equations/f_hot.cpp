...
void SimulationGenerator::ConstructEquation_f_hot_kineq(
    ...
    if(ava_mode == OptionConstants::EQTERM_AVALANCHE_MODE_KINETIC_RP) {
        ...
        if (eqsys->HasRunawayGrid()){
            Op_ava->AddTerm(new AvalancheSourceCH(hottailGrid, eqsys->GetUnknownHandler(), pCutoff, -1.0, AvalancheSourceCH::CH_SOURCE_MODE_KINETIC, false, eqsys->GetRunawayGrid()));
        } else{
            Op_ava->AddTerm(new AvalancheSourceCH(hottailGrid, eqsys->GetUnknownHandler(), pCutoff, -1.0, AvalancheSourceCH::CH_SOURCE_MODE_KINETIC, false));
        }
