 ...
 * Constructor.
 */
KineticEnergyFromDistributionFunction::KineticEnergyFromDistributionFunction(
        ...
        for(len_t ir = 0; ir<rGrid->GetNr(); ir++){
            ...
            for(len_t ip1 = 0; ip1<np1; ip1++){
                for(len_t ip2 = 0; ip2<np2; ip2++){
                    ind = ip2*np1+ip1;
                    this->integrand[offset+ind] = scaleFactor * Constants::me*Constants::c*Constants::c*(mg->GetGamma(ip1,ip2)-1);
                }
            }
            offset += np1*np2;
        }
