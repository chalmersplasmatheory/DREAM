 ...
 * interact with waves injected into a tokamak.
 */
...
WavePitchScattering::WavePitchScattering(
    ...
    DREAM::MultiInterpolator1D *ppar_res, DREAM::MultiInterpolator1D *Delta_ppar_res,
    DREAM::MultiInterpolator1D *Dxx_int
) : WavePitchScattering(
...
void WavePitchScattering::Rebuild(const real_t t, const real_t, FVM::UnknownQuantityHandler*) {
    ...
    for (len_t ir = 0; ir < nr; ir++) {
        ...
        // get diffusion strength at this radius
            ...
            for (len_t i = 0; i < np; i++) {
                ...
                if (mode == OptionConstants::EQTERM_WAVE_MODE_GAUSSIAN){
                    if (absxi != 0) {
                        ...
                        D22(ir, i, j_f) += Dxx_int0 * exp(-exp_nom/exp_denom) / sqrt(exp_denom*M_PI);
                    }
                }
            }
        }
