...
void OtherQuantityHandler::DefineQuantities() {
    ...
    if (tracked_terms->T_cold_ohmic != nullptr)
        ...
        if (tracked_terms->T_cold_parallel != nullptr)
        DEF_FL("fluid/Tcold_parallel", "Parallel power density [J s^-1 m^-3]",
            real_t *Wcold = this->unknowns->GetUnknownData(this->id_Wcold);
            real_t *vec = qd->StoreEmpty();
            for(len_t ir=0; ir<this->fluidGrid->GetNr(); ir++)
                vec[ir] = 0;
            this->tracked_terms->T_cold_parallel->SetVectorElements(vec, Wcold);
    );
