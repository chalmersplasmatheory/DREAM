...
class ChargeStateRate {
    public:
        ...
        virtual real_t Eval(const len_t Z0, const real_t n, const real_t T) const = 0;

        virtual real_t Eval_deriv_n(const len_t Z0, const real_t n, const real_t T) const {
            real_t eps = std::numeric_limits<real_t>::epsilon();
            real_t dn = sqrt(eps)*n + eps;
            return (Eval(Z0, n+dn, T) - Eval(Z0, n, T)) / dn;
        }

        virtual real_t Eval_deriv_T(const len_t Z0, const real_t n, const real_t T) const {
            real_t eps = std::numeric_limits<real_t>::epsilon();
...
class ZeroChargeStateRate : public ChargeStateRate {
    ...
    public:
        ...
        virtual real_t Eval(const len_t, const real_t, const real_t) const override {
                return 0;}
        virtual real_t Eval_deriv_n(const len_t, const real_t, const real_t) const override {
                return 0;}
        virtual real_t Eval_deriv_T(const len_t, const real_t, const real_t) const override {
                return 0;}
...
class TabledChargeStateRate : public ChargeStateRate {
    ...
    public:
        ...
        virtual real_t Eval(const len_t Z0, const real_t n, const real_t T) const override {
                if (Z0 != activeZ0)
                    return 0;
                ...
                printf("TabledChargeStateRate::Eval: Evaluating rate '%s' for Z0=%d, n=%e, T=%e gives %e.\n", name.c_str(), Z0, n, T, std::exp(sum) * 1e-6);
                return std::exp(sum) * 1e-6; // Factor 1e6 converts from cm^3 to m^3

        }
        //Implement the derivatives which should be easy, look at AMJUEL.cpp
        };
    }
  #endif
