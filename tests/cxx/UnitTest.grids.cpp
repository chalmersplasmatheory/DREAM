 ...
 */
TQS::FVM::RadialGrid *UnitTest::InitializeGeneralGridPXi(const len_t nr, const len_t np, const len_t nxi) {
    const real_t B0 = 2;
    const real_t pMin = 0, pMax = 10;

    auto *crgg = new TQS::FVM::CylindricalRadialGridGenerator(nr, B0);
    auto *rg = new TQS::FVM::RadialGrid(crgg);
