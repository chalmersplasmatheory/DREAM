...
real_t AnalyticBRadialGridGenerator::GeneralBounceAverageTestFunction(real_t x, real_t y, real_t r, real_t t, void*){
//    return 0.315*pow(abs(x),1.382)*pow(y,2.913)*pow(r,0.592)*pow(t,0.192);
    return 0.315*pow(abs(x),1.382)*pow(y,2.913)*pow(r,0.474)*pow(t,0.812);
}
real_t AnalyticBRadialGridGenerator::GeneralFluxSurfaceAverageTestFunction(real_t x, real_t y, real_t z, void*){
...
bool AnalyticBRadialGridGenerator::TestGeneralBounceAverage(){
    ...
    for(len_t it=0; it<Ntests; it++){
        ...
        if(fluxGridTypes[it]==DREAM::FVM::FLUXGRIDTYPE_P2)
            ...
            xi0 = grid->GetMomentumGrid(0)->GetP2(j);
        real_t BA = grid->GetRadialGrid()->CalculatePXiBounceAverageAtP(ir,xi0,fluxGridTypes[it],GeneralBounceAverageTestFunction);
...
bool AnalyticBRadialGridGenerator::TestGeneralFluxSurfaceAverage(){
    ...
    real_t generalFluxSurfaceAverage1 = grid->GetRadialGrid()->CalculateFluxSurfaceAverage(0,DREAM::FVM::FLUXGRIDTYPE_DISTRIBUTION,&GeneralFluxSurfaceAverageTestFunction);
    real_t referenceValueMatlab1 = 0.65482084231139;

    real_t generalFluxSurfaceAverage2 = grid->GetRadialGrid()->CalculateFluxSurfaceAverage(3,DREAM::FVM::FLUXGRIDTYPE_RADIAL,&GeneralFluxSurfaceAverageTestFunction);
