...
def gensettings(T, Z=1, EEc=1e-2, n=5e19, yMax=5):
    ...
    ds.radialgrid.setShaping(psi=psiref, rpsi=rref, G=2.0)
...
def runTZ(T, Z):
    ...
    epf = do.grid.effectivePassingFraction
...
def run(args):
    ...
    T, Z, CODEsigma = loadCODE('{}/../code_conductivity/CODE-conductivities.mat'.format(workdir))

    nZ, nT = T.shape
    sigma = np.zeros((nZ, nT, NR))
    effectivePassingFraction = None
    for i in range(0, nZ):
        for j in range(0, nT):
            print('Checking T = {} eV, Z = {:.1f}... '.format(T[i,j], Z[i,j]), end="")
            try:
                sigma[i,j,:], effectivePassingFraction = runTZ(T[i,j], Z[i,j])
            ...
            Delta = np.amax(np.abs(sigma[i,j,:] / (CODEsigma[i,j]*effectivePassingFraction) - 1.0))
