 ...
 */
XiUniformThetaGridGenerator::XiUniformThetaGridGenerator(
    const len_t nxi
) : nxi(nxi) {
    
    if (nxi < 1)
        throw MomentumGridGeneratorException(
            "Uniform theta grid generator: A uniform grid must contain at least 1 cell. Specified number of cells: " LEN_T_PRINTF_FMT ".", nxi
        );

    if (xiMin == xiMax)
        throw MomentumGridGeneratorException(
            "Uniform theta grid generator: The two endpoints of a uniform grid may not be the equal."
        );

    if (xiMin > xiMax)
        throw MomentumGridGeneratorException(
            "Uniform theta grid generator: The lower boundary 'xiMin' of a uniform grid must be located below 'xiMax'."
        );
}
 ...
 */
bool XiUniformThetaGridGenerator::Rebuild(const real_t, const len_t, MomentumGrid *mg, const RadialGrid*) {
    real_t
