...
real_t SPIHandler::delta_r_limit(int ip){
    return v_lab * t_acc + M_PI*n*T[irp[ip]]*Reff*q/(B*B*CST);
}

std::complex<real_t> SPIHandler::E_i(std::complex<real_t> z){
...
real_t SPIHandler::third_row(){
    real_t factor = t_pe * 1/(1 + 1/(t_polp*t_polp));
    real_t term1 = primitive_third_row(t_pep);
    real_t term2 = primitive_third_row(0);
    return factor * (term1 - term2);
}

real_t SPIHandler::delta_r(){
    real_t first = first_row();
    real_t second = second_row();
    real_t third = third_row();
    real_t term1 = v_lab * t_acc;
    real_t factor = (1+Zavg)*2*qe*T*q/(CST*(mD*fD[ip]+mNe*fNe[ip]));
    return term1 + factor * (first + second + third);
}

void SPIHandler::assign_time_parameters(){
    real_t t_acc = n/(1+Zavg)*(mD*fD+mNe*fNe)*Reff/(B*B);
    real_t t_pol = q*Rm/CST;
    real_t t_pe = qe*T*n/(2*CST*(n_i+n_e)*qe*Te);
    real_t t_exp = Lc/(2*CST);
    real_t t_polp = t_pol/t_acc;
    real_t t_pep = t_pe/t_acc;
    real_t t_expp = t_ep/t_acc;
}

void SPIHandler::assign_misc_parameters(){
    //free parameters
    T[2] = {0.5, 0.5};
    T_0 = 1;
    delta_y = 1;
    ...
    T0NGS = 2e3;
    rp0NGS = 2e-3;
    q0NGS = sqrt(2 * T0NGS * T0NGS * T0NGS/(M_PI * me));
    E0NGS = 2*T0NGS;
    //other
    v0 = vp;
    n_e = ntot;
...
void SPIHandler::compute_parameters(int ir){
    B = sqrt(this->rGrid->GetFSA_B2(irp[ip]));
    Te = Tcold[irp[ip]];
    Zavg = ZavgD*fD[ip] + ZavgNe*fNe[ip];
    CST = sqrt((gamma_e*Zavg + gamma_i) * qe * T/(mD*fD[ip] + mNe*fNe[ip]));
    CST0 = sqrt((gamma_e*Zavg + gamma_i) * qe * T_0/(mD*fD[ip] + mNe*fNe[ip]));
    qin = ne * sqrt((gamma_e * Zavg + gamma_i) * qe * Te/(mD * fD[ip] + mNe * fNe[ip]));
    G = NGSConstantFactor[irp[ip]];
    n_0 = (1 + Zavg)*G/(2 * M_PI * delta_y * delta_y * (mD*fD[ip] + mNe*fNe[ip])*CST0);
    a0 = ((1 + Zavg0)*qe*T_0/((mD*fD[ip] + mNe * fNe[ip])*Rm));
    t_detach = -v0/a0 + sqrt(v0*v0/(a0*a0) + 2 * delta_y/a0);
    Lc = 2 * CST0*t_detach;
    n = n_0 * Lc;
    v_lab = a0 * t_detach;
    lnLambda = CoulombLogarithm->evaluateLnLambdaT(irp[ip]);
    sigma = eqsys->GetREFluid()->GetElectricConductivity(irp[ip]);
    Reff = -2*M_PI*M_PI*r/(sigma*delta_y*delta_y*delta_y*std::log((delta_y/(r*M_PI))));
}

void SPIHandler::Rebuild(real_t dt){
    for(int ip = 0; ip < len(fD); ip++){
        compute_parameters();
        real_t result[ip] = delta_r();
    ...
    if(spi_deposition_mode==OptionConstants::EQTERM_SPI_DEPOSITION_MODE_LOCAL){
        ...
        for(len_t ip=0;ip<nShard;ip++){
            if(rCoordPNext[ip]>rCoordPPrevious[ip]){
                for(len_t ir=0;ir<nr-1;ir++){
                    depositionProfilesAllShards[ir*nShard+ip]=rGrid->GetVpVol(ir+1)/rGrid->GetVpVol(ir)*depositionProfilesAllShards[(ir+1)*nShard+ip] + result[ip];
                }
            }else if(rCoordPNext[ip]<rCoordPPrevious[ip]){
                for(len_t ir=nr-1;ir>0;ir--){
                    depositionProfilesAllShards[ir*nShard+ip]=rGrid->GetVpVol(ir-1)/rGrid->GetVpVol(ir)*depositionProfilesAllShards[(ir-1)*nShard+ip] + result[ip];
